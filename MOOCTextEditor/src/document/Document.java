...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i = 0;i<word.length();i++) {
			...
			if(i==word.length()-1 && word.charAt(i)=='e' && flag && syllableCount>0) syllableCount--;
			
			if(flag && vow.indexOf(word.charAt(i))!=-1) {
				flag = false;
				syllableCount++;
			}
			else if(vow.indexOf(word.charAt(i))==-1)
				flag = true;
			}
	...
	public double getFleschScore()
	{
		...
		double flesch = 206.835-1.015*(1.0*getNumWords()/getNumSentences())-84.6*(1.0*getNumSyllables()/getNumWords());
	    return flesch;
	}
